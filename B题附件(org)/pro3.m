...
% 读取数据  
 data = readtable('C2.csv');  
% data = readtable('C2.csv');  
 %data = readtable('C3.csv');  
% data = readtable('C4.csv');  
...
T_green_record = [];
% 设定速度阈值，用于判断车辆是否停下  
speed_threshold = 0.1; % 例如，0.5 m/s  
...
for i = 1:length(vehicle_ids)
    ...
    % 如果车距离信号灯太远，则跳过
    if abs(vehicle_x(stop_indices(1)) -linex)>20 && abs(vehicle_y(stop_indices(1)) -liney)>20
        continue;  
    end  
    ...
    % 如果停止时间太短的点，则跳过此车辆  
    if  length(stop_indices)<2
        green_start = green_start_last;
        continue;  
    end 

    if  i >1 && T_red_start < T_red_start_last 
        T_signal = [T_signal(1:end-1) , green_start - green_start_last];   %总周期T = T_green - 上一次的T_green
       T_red = [T_red(1:end-1), T_red_current];  
       T_green = [T_green(1:end-1), T_green_current];  
       T_green_record = [T_green_record(1:end-1),green_start];
    ...
    T_green = [T_green, T_green_current];  
     T_green_record = [T_green_record,green_start];
...
T_circle = [t,(T_signal)',T_red',T_green',T_green_record'];  
...
disp(['估计的红灯时长（秒）: ', num2str(max(T_red))]);  
...
%% Pettitt突变点检测
figure(8)
plot(T_circle(:,5),T_circle(:,2));
xlabel('时间');
ylabel('信号灯周期时长')
ylim([20,200]);
title(['时间-信号灯周期时长']);
T_sig = T_circle(:, 2);  % 提取红灯时间序列

% 初始化存储结果的变量
change_points = [];

for i = 2:length(T_sig)-2
    if abs(T_sig(i) - T_sig(i-1)) > 50 && abs(T_sig(i) - T_sig(i+1)) < 10 && abs(T_sig(i) - T_sig(i+2)) < 10
        change_points = [change_points, [T_circle(i,5); T_circle(i,3);T_circle(i,4);T_circle(i,2)]];
    end
end
k = size(change_points);
% 输出检测结果
if ~isempty(change_points)
    fprintf('检测到以下突变点：\n');

    for i = 1:k(2)
        fprintf('突变点时间：%d红灯时间：%d\n绿灯时间：%d\n 周期时间：%d\n', change_points(1,i), change_points(2,i), change_points(3,i),change_points(4,i));
    end
else
    fprintf('未检测到符合条件的突变点。\n');
end
%%
%{
% 获取信号灯周期 T_signal
...
T_change = [];  % 存储检测到的周期变化信息

n = length(T_signal);  % 计算信号灯周期的个数
critical_value = 0.05;  % 设置显著性水平对应的临界值

% 初始化变量
tau = zeros(n, 1);  % 存储Pettitt检测的统计量
K = zeros(n, 1);    % 存储Pettitt检测的累积量
T_flag = 0;  % 记录累积的突变点位置

% 计算Pettitt检测的统计量和累积量
for t = 2:n
    if t > length(T_signal)
        break;
    end
    
    K(t) = max(T_signal(1:t)) - min(T_signal(1:t));
    tau(t) = sum(sign(T_signal(t) - T_signal(1:t-1)));
    
    % 计算Pettitt检测的统计量的绝对值
    abs_tau = abs(tau);
    
    % 计算Pettitt检测的检测量
    U_t = abs_tau - max(abs_tau) / 2;
    ...
    % 输出检测结果
    if U_t(idx) > critical_value
        T_change = [T_change, [T_circle(T_flag, 5); T_red(T_flag); T_green(T_flag); T_signal(idx)]];
        fprintf('发现信号灯周期发生突变！\n');
        fprintf('突变发生在第 %d s。\n', T_circle(T_flag, 5));
        fprintf('新周期参数为:\n T_signal(%d) = %f\n', T_flag, T_signal(idx));
        fprintf('红灯时长为: T_red(%d) = %f\n', T_flag, T_red(T_flag));
        fprintf('绿灯时长为: T_green(%d) = %f\n', T_flag, T_green(T_flag));
        fprintf('旧周期参数为:\n T_signal(%d) = %f\n', idx-1, T_signal(idx-1));
        fprintf('红灯时长为: T_red(%d) = %f\n', T_flag-1, T_red(T_flag-1));
        fprintf('绿灯时长为: T_green(%d) = %f\n', T_flag-1, T_green(T_flag-1));
        
        % 更新信号灯周期序列，继续检测后续的周期变化
        T_signal = T_signal(idx+1:end);
        n = length(T_signal);
        tau = zeros(n, 1);
        K = zeros(n, 1);
...
for subplot_index = 1:num_subplots
    ...
    for i = start_index:end_index
        ...
        plot(vehicle_data.time, vehicle_data.x, '-', 'DisplayName', ['Vehicle ' num2str(vehicle_ids(i))]);
    end
