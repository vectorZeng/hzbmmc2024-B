...
T_circle = [t,(T_signal)',T_red',T_green',waiting_vehicel'];  
...
% 数据处理->删除离群值
T_circle = rmoutliers(T_circle,"DataVariables","T_circle2");
...
figure(8)
    ...
    n =size(T_circle);
    for j = 1:n(1)
        vehicle_data = data(data.vehicle_id == T_circle(j,5), :);
        plot(vehicle_data.time, vehicle_data.v, '-', 'DisplayName', ['Vehicle ' num2str(T_waiting_vehicel(j,1))]);
    end
    
    xlabel('时间(s)');
    ylabel('车辆速度（m/s）');
    title(['车辆时间-速度图(processed)']);
      ylim([-5,20]);
    legend('show');
